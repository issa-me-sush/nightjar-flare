# The enclave image, for Railway.
#
# Byte-for-byte the same build as go/Dockerfile with one difference: the two
# apt cache mounts are gone. Railway's builder requires cache mount ids to
# carry its own cache-key prefix, and go/Dockerfile is the reproducible build
# whose output becomes the code hash registered on-chain — it should not be
# bent around one host's build system. Cache mounts only make builds faster;
# removing them changes nothing about what is produced.
#
# Keep this in step with go/Dockerfile when that changes.
